...
def predict():
    ...
    if not source or not destination or not departure_time:
        ...
        return jsonify({
            "error":
                "Source, destination and departure time are required"
        }), 400
    ...
    distance_km = route_result[
        "distance_km"
    ]

    duration_minutes = route_result[
        "duration_minutes"
    ]

    route_points = route_result[
        "route_points"
    ]

    source_coordinates = route_result[
        "source_coordinates"
    ]

    destination_coordinates = route_result[
        "destination_coordinates"
    ]
    ...
    major_roads = route_result[
        "major_roads"
    ]

    state_roads = route_result[
        "state_roads"
    ]

    other_roads = route_result[
        "other_roads"
    ]

    roundabouts = route_result[
        "roundabouts"
    ]

    service_roads = route_result[
        "service_roads"
    ]

    flyovers_underpasses = route_result[
        "flyovers_underpasses"
    ]

    asphalt_percentage = route_result[
        "asphalt_percentage"
    ]

    paved_percentage = route_result[
        "paved_percentage"
    ]

    concrete_percentage = route_result[
        "concrete_percentage"
    ]

    good_surface_percentage = route_result[
    ...
    weather_result = get_overall_route_weather(
        route_points
    )
    ...
    route_prediction = min(
        max(route_prediction, 0),
        1
    )
...
# =====================================================
    ...
    if risk_level == "High":
        ...
        alert = (
            "✅ Low-risk conditions on this route."
        )
    ...
    return jsonify({
        ...
        "message":
            "Route safety analysis completed using live route weather and ML"
